...
- [ ] 【オーナー承認待ち・設計提案書あり】**写真のAIモデレーション導入**。提案書: `docs/design/photo_ai_moderation_proposal.md`（プロバイダA案=OpenAI推奨。確認質問3件あり）
- [ ] 【オーナー承認待ち・設計提案書あり】**いいね一言メッセージの暴言・誹謗中傷フィルタ**。調査の結果、一言メッセージ機能自体は実装済みと判明（tasks.mdの旧記述は誤り）。未実装なのは専用NGワード辞書のみ。提案書: `docs/design/abuse_word_filter_proposal.md`（確認質問3件あり）
