...
- データ: `likes where to_user = me` + 送り主profile（相性計算にも使用）
  - **2026-09-26 I15**: 取得は RPC `get_received_likes_page()`（表示日の割当を DB 側で全件に対して行い、新しい順に50件ずつページ化。繰越件数は全体の値）。下記の `assignVisibleDates` は同じアルゴリズムの TS 版（仕様としてテストを残す）。設計: `docs/design/2026-09-26_I15_設計提案.md`
