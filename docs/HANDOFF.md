...
- **現在地**: **M0〜M5 完了**（動く）。残るは **M6（課金モック・トーン統一・仕上げ）のみ**。
- **リポジトリ**: `C:\Users\haosh\dev\hapimari`（**日本語パス禁止の事情でここに置いてある。§3参照**）
- **次にやること**: M6設計書 → オーナー承認 → 実装。M6には通常スコープ（課金モック・R9・
  daily_stats・透明性レポート・退会）に加え、**トーン統一（中高年45-65）・子持ち表示の扱い（R3含む）・
  女性プレミア500円将来枠・価値観マッチング強化案**を含める（decisions 2026-07-06/07参照）。
- 受け入れ記録: `docs/acceptance/M3.md` / `M4.md` / `M5.md`。
...
| **M6** | **課金モック・透明性レポート・退会 + トーン統一等の追加分** | **未着手（次はここ・設計書から）** |
