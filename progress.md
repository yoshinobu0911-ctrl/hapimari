...
- 2026-09-24 (Claude): **複数AI（Claude Code / Codex / Grok）の作業がぶつからない構成かを点検**（オーナー依頼・クラウドセッション）。結論: 現状は**全AIが同じフォルダ・同じブランチ（main）で作業**しており、同時に動かすとファイル・コミット・ローカルDBが混ざる構成だった。開発憲法（CLAUDE.md）も Codex 等には自動で届かず、共通ルール `dev\AGENTS.md` はリポジトリの外にある。コミットの半数（52/105）はどのAIのものか判別できない。対策として、**リポジトリ直下に `AGENTS.md`（全AI共通ルール）を新設**し、CLAUDE.md §2/§3 を最小限修正、`.gitignore` に `.claude/worktrees/` を追加した（ブランチ `claude/inspiring-euler-acq15n`・main には未統合）。AIごとの作業フォルダ（git worktree）の作成はローカルでしかできないため、指示書 `docs/handoff/2026-09-24_multi-agent-setup.md` に分けた → `docs/decisions/2026-09-24_複数AIの作業分離.md`（オーナー承認待ち）
