...
4. **法定表示のリンク行は未設置**（§9-3 保留の判断による）。法定4行の脚注のみ実装。
   ...
   - **【2026-08-26 追記・解消済み】** リンク行は設置完了（`apps/mobile/src/app/subscription.tsx:412-423`）。
     特商法・利用規約・プライバシーポリシーの3本を `https://www.happymarry.jp/` 配下へ、
     contract中／解約予約中／プラン選択の**全状態で常時表示**する形で実装した。
     残る課題はリンク先ページの**本文が仮文面のまま**であること（`docs/launch_checklist.md` #25）。
...
7. 受け入れ条件 §11-(4)（リンクが機能）は §9-3 保留により**対象外に変更**。
   - **【2026-08-26 追記】** 上記4のとおりリンクを設置したため、§11-(4) は再び対象。
     ただし公開前の最終確認は、リンク先が正式文面に差し替わってから行う。
