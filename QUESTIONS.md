...
### Q9. ブロック解除一覧に相手のニックネームを表示できない（M3・暫定判断）
- ブロック中は §3.3 のRLS（両方向遮断）により**相手のプロフィール行そのものが取得できない**ため、
  「ブロックしたユーザー」一覧（マイページ→設定）は**ブロックした日時のみ**表示している。
- 改善案: (a) ブロック時に blocks へニックネームを非正規化保存（スキーマ変更）
  (b) blocker本人にのみニックネームを返す security definer RPC を追加。
- どちらもセンシティブ領域（RLS/個人情報）のため**オーナー承認後に実施**。MVPでは日時のみで運用。
