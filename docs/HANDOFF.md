...
> **【2026年7月時点の記録。今の手順ではありません】（2026-09-24 追記）**
> 最新の現在地は `HANDOFF.md`（リポジトリ直下）と `progress.md`、作業ルールは `AGENTS.md`（開発憲法）にある。
> この文書の手順や指示が `AGENTS.md` と食い違う場合は、`AGENTS.md` に従うこと。
...
- **現在地**: 🎉 **M0〜M6 すべて完了＝MVP機能完成**（2026-07-07）。
- **リポジトリ**: `C:\Users\haosh\dev\hapimari`（**日本語パス禁止の事情でここに置いてある。§3参照**）
- **次にやること**: ①業務委託エンジニアによる**MVP完成時レビュー**（3段階レビューB・
  資料は `docs/review/2026-07-07_MVPレビュー引き継ぎ.md`） ②デザイナーUI刷新の反映
  （要件定義書: `docs/design/designer_brief.md`） ③本番公開準備（管理画面認証・環境変数・
  届出等の人間タスク。QUESTIONS Q4/Q5/Q17参照） ④公開後にM7（行動データの推薦）。
- 受け入れ記録: `docs/acceptance/M0〜M6.md`。暫定判断: `QUESTIONS.md` Q1〜Q19。
- M6の主な追加: 課金モック(R9)・透明性レポート・退会・**距離マッチング（30km既定・
  座標はprofile_locationsに分離して完全秘匿）**・子持ち表示撤去（R3は表示除外方式に変更）・
  中高年45-65トーン・相性理由の言語化・女性プレミア500円将来枠。
- **重要な製品方針（2026-07-06/07 オーナー指示）**: 中核ターゲットは**中高年45〜65の男女**
  （marrish型のバツイチ子持ち特化ではない）。子持ち表示の扱い・トーン統一・女性プレミア500円将来枠・
  価値観マッチング強化（診断なし）は**M6でまとめて設計**（docs/decisions/ の2026-07-06/07参照）。
...
1. **日本語パス禁止**: Supabase CLI は日本語を含むパス（OneDrive/デスクトップ等）で **exit 0 のままサイレント失敗**する。作業は `C:\Users\haosh\dev\` 配下の英数字だけのパスで行う（本体 `hapimari` と、AIごとの作業フォルダ。割り当ては `AGENTS.md` §13-1）。
...
| M4 | デート移行支援（差別化の核） | ✅ 完了（受け入れ: docs/acceptance/M4.md） |
| M5 | 音声通話（モックSDK） | ✅ 完了（受け入れ: docs/acceptance/M5.md） |
| M6 | 課金モック・透明性レポート・退会 + トーン統一・距離マッチング | ✅ 完了（受け入れ: docs/acceptance/M6.md）**→ MVP完成** |
...
M3 実装用だった旧プロンプトは、M3 が完了済みで今の作業ルールとも食い違うため削除した（2026-09-24。原文は git の履歴にある）。

今は、AIを `AGENTS.md` §13-1 の表にある作業フォルダで起動すれば、Claude Code と Codex は開発憲法（`AGENTS.md`）を自動で読む。念のため、最初の依頼の前に次の1行を添える:

```
AGENTS.md（開発憲法）を全文読み、今いるフォルダが §13-1 の表にある自分の作業フォルダか確認してから（違っていたら作業せずに知らせる）、progress.md → tasks.md で現在地を確認してください。
```
