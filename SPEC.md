...
| ID | ルール | 実装箇所 |
|---|---|---|
| R1 | 登録可能年齢: **男女とも35歳以上**（上限なし）※2026-07-12 オーナー決定で男性45→35に改定（年齢で強く区切らない方針・ラス恋型にしない） | サインアップバリデーション + DB check |
...
> **注記（2026-09-26・統合表 I31）**: この節は**当初計画（歴史的記述）**。受け入れ条件の一部はその後のオーナー決定で変わっており、**現行のテスト条件として使わないこと**。現行の条件は各マイルストーン設計書（`docs/design/M{n}_design.md`）と受け入れ記録（`docs/acceptance/`）が正。主な変更点は各行の「現行:」に記した。
...
（現行: R1 は2026-07-12 決定で**男女とも35歳以上**。44歳男性は登録できる。拒否されるのは34歳以下）
...
（現行: R3 は2026-07-12 決定で廃止。子持ち女性への理解宣言なし男性のいいねは**エラーにならない**。いいねは送信者の本人確認後のみ＝2026-08-26 決定）
...
（現行: R5 は2026-07-12 決定で廃止。**メッセージ数による解禁は無い**。R6 の両者合意ロジックは現行どおり）
...
（現行: 通話はマッチ直後から可能（メッセージ数の解禁なし）。ただし**双方の本人確認が前提**（2026-08-26 決定）。実装は M8 で Agora 本実装済み（`supabase/functions/agora-token`・`docs/design/M8_call_design.md`）。実音声の疎通は Agora キー設定後に確認）
...
（現行: 課金は M7.x で **Stripe 本実装**（Checkout＋Webhook。`docs/design/M7_2_payment_ui_design.md`）。モック課金コードは削除済み。判定は `is_subscription_active(auth.uid())`。実 Stripe キーでの疎通は未実施）
...
| 依存 | MVPでの扱い |
|---|---|
| 決済（ストア課金/RevenueCat） | `PaymentProvider` インターフェース + 常にsuccessを返すモック。実装差し替えポイントをREADMEに明記（**現行: Stripe 本実装に移行済み。この行は当初方針**） |
| eKYC（本人確認の自動照合） | 目視審査運用とし、自動照合はなし。管理画面の審査キューが正式フロー |
| 音声通話SDK（Agora） | `CallProvider` インターフェース + 最小WebRTC/ダミー。APIキーは env.example にプレースホルダ（**現行: Web は Agora 本実装済み。App ID と証明書は Edge Function の環境変数だけに置き、アプリはトークン応答で受け取る。ネイティブは引き続きモック**） |
