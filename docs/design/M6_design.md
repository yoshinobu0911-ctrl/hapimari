...
- **位置情報を許可していないユーザーの扱い（2026-07-07 オーナー最終決定）**:
  県庁所在地フォールバックは**採用しない**。位置未許可ユーザー（またはそのペア）は
  ①距離を計算・表示しない ②**「距離が近い順」ソートは使えない**（トグルを無効化し
  「位置情報を許可すると距離順に並べ替えできます」と案内） ③相性スコアは**距離を除いた
  重みで再正規化**して他の価値観で評価 ④表示範囲は「**同一県なら常に表示**」の救済則
  （県は全員必須入力のため母集団が空にならない）。
